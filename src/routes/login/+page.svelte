<script lang="ts">
	...
	// Unlock mode: session exists but vault is locked (e.g. after page refresh)
	// The vaultSalt is already available from the load — no server round-trip needed.
	...
	// Login mode: no session — server validates credentials, returns salt.
	const handleLogin = () => {
...
<main class="min-h-screen flex items-center justify-center p-4">
	<div class="w-full max-w-md">
		...
		<div class="glass-strong rounded-2xl p-8">
			...
			{#if data.isUnlock}
				<!-- Unlock mode — purely client-side key derivation -->
				<form onsubmit={handleUnlock}>
					<div class="mb-4">
						<p class="text-xs text-muted mb-3">Signed in as <span class="text-lilac">{data.email}</span></p>
