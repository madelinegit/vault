		...
		<div class="glass rounded-2xl p-5 mb-6 flex gap-3 items-end">
			<div>
				...
				<input
					...
					bind:value={newIcon}
					maxlength="2"
					class="input-glass w-16 px-3 py-2 rounded-lg text-xl text-center"
				/>
			</div>
			<div class="flex-1">
				<label class="block text-xs text-muted mb-1.5">Category Name</label>
				<input
					type="text"
					bind:value={newName}
			...
			<button onclick={addCategory} disabled={addLoading} class="btn-primary px-5 py-2.5 rounded-xl text-sm">
				{addLoading ? '…' : 'Add'}
			</button>
			<button onclick={() => (showAddCategory = false)} class="btn-ghost px-4 py-2.5 rounded-xl text-sm">
				Cancel
			</button>
		</div>
	{/if}
