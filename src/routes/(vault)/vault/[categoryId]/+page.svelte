<script lang="ts">
	...
	async function saveItem(name: string, fields: VaultField[]) {
		...
		if (res.ok) {
			...
			decryptedItems = [
				...decryptedItems,
				{ id: saved.id, categoryId: data.category.id, name, fields, sortOrder: saved.sortOrder }
			];
			showNew = false;
		}
	}
	...
	async function updateItem(id: string, name: string, fields: VaultField[]) {
		...
		decryptedItems = decryptedItems.map((item) =>
			item.id === id ? { ...item, name, fields } : item
		);
	}
...
<div>
	<div class="flex items-center gap-3 mb-8">
		<button onclick={() => goto('/vault')} class="btn-ghost px-3 py-2 rounded-xl text-sm">
			← Back
		</button>
		<div>
		...
		<button
			...
			class="btn-primary ml-auto px-5 py-2.5 rounded-xl text-sm"
		>
			+ Add Item
		</button>
	</div>
	...
	{#if decryptError}
		<div class="mb-6 px-4 py-3 rounded-lg text-sm" style="background: rgba(239,68,68,0.12); border: 1px solid rgba(239,68,68,0.25); color: #FCA5A5;">
			{decryptError}
		</div>
	{/if}
